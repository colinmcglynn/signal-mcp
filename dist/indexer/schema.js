...
export function initSchema(db) {
    db.exec(FTS_DDL);
    const cur = db.prepare(`SELECT version FROM schema_version LIMIT 1`).get();
    if (!cur) {
        db.prepare(`INSERT INTO schema_version (version) VALUES (?)`).run(FTS_SCHEMA_VERSION);
    }
    else if (cur.version !== FTS_SCHEMA_VERSION) {
        throw new Error(`FTS DB schema version ${cur.version} does not match expected ${FTS_SCHEMA_VERSION}. ` +
