...
export function assertSourceSchema(signalDb) {
    const cols = signalDb.prepare(`PRAGMA table_info('messages')`).all();
    const have = new Set(cols.map((c) => c.name));
    const missing = REQUIRED_MESSAGE_COLS.filter((c) => !have.has(c));
    if (missing.length > 0) {
        throw new Error(`Signal messages table is missing expected columns: ${missing.join(', ')}. ` +
    ...
    const edited = signalDb
        ...
        .get();
    if (!edited) {
        throw new Error(`Signal database is missing 'edited_messages' table. Schema may have changed; run scripts/probe.ts.`);
    }
}
function readWatermark(ftsDb) {
    const row = ftsDb
...
function writeWatermark(ftsDb, wm, stats) {
    ftsDb
        .prepare(`UPDATE sync_state
...
function upsert(ftsDb, row) {
    const existing = ftsDb.prepare(`SELECT 1 FROM messages WHERE guid = ?`).get(row.id);
    ftsDb
        .prepare(`DELETE FROM messages_fts WHERE guid = ?`)
        .run(row.id);
    ftsDb
        .prepare(`INSERT OR REPLACE INTO messages (guid, body, chat_id, sender_id, sent_at, received_at)
       VALUES (?, ?, ?, ?, ?, ?)`)
        .run(row.id, row.body, row.conversationId, row.sourceServiceId, row.sent_at, row.received_at);
    ftsDb
        .prepare(`INSERT INTO messages_fts (body, guid, chat_id, sender_id, sent_at, received_at)
       VALUES (?, ?, ?, ?, ?, ?)`)
        .run(row.body, row.id, row.conversationId, row.sourceServiceId, row.sent_at, row.received_at);
    return existing ? 'updated' : 'inserted';
}
function deleteRow(ftsDb, guid) {
 ...
 */
export function syncOnce(signalDb, ftsDb, { backfill = false, progress } = {}) {
    ...
    const fetchBatch = signalDb.prepare(`SELECT id, conversationId, sent_at, received_at, sourceServiceId, body
     FROM messages
    ...
    while (true) {
        ...
        if (rows.length === 0)
            break;
        const tx = ftsDb.transaction((batch) => {
        ...
        batches++;
        if (progress)
            progress(`forward: ${inserted + updated} processed (last sent_at=${wm.sent_at})`);
    }
    ...
    const editIds = signalDb
        ...
        .all();
    if (editIds.length > 0) {
        const getCurrent = signalDb.prepare(`SELECT id, conversationId, sent_at, received_at, sourceServiceId, body
       FROM messages
       WHERE id = ?
         ...
         AND (isErased = 0 OR isErased IS NULL)
         AND type IN ('incoming','outgoing')`);
        const tx = ftsDb.transaction((ids) => {
            for (const { messageId } of ids) {
                const row = getCurrent.get(messageId);
                if (!row)
                    continue;
                const existingBody = ftsDb
                    .prepare(`SELECT body FROM messages WHERE guid = ?`)
                    .get(row.id);
                if (existingBody && existingBody.body === row.body)
                    continue;
                const which = upsert(ftsDb, row);
                if (which === 'inserted')
                    inserted++;
        ...
        tx(editIds);
        if (progress)
            progress(`edits: reconciled ${editIds.length} candidates`);
    }
    ...
    const erased = signalDb
        ...
        .all();
    if (erased.length > 0) {
        const tx = ftsDb.transaction((rows) => {
            for (const { id } of rows) {
                if (deleteRow(ftsDb, id))
                    deleted++;
            }
        });
        tx(erased);
    ...
    const ourGuids = ftsDb.prepare(`SELECT guid FROM messages`).all();
