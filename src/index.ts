...
): (args: TArgs) => Promise<ReturnType<typeof jsonResult> | ReturnType<typeof errorResult>> {
  return async (args) => {
...
async function main() {
  ...
  const server = new McpServer(
    { name: 'signal-mcp', version: '0.1.0' },
    {
      capabilities: { tools: {} },
      instructions:
        "Read-only access to Signal Desktop's local message database. " +
