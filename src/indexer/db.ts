...
export interface FtsDb {
  db: Database.Database;
  path: string;
}

export function defaultFtsDbPath(): string {
  if (process.env.SIGNAL_MCP_FTS_DB) return process.env.SIGNAL_MCP_FTS_DB;
...
export function openFtsDb(path = defaultFtsDbPath(), { readOnly = false } = {}): FtsDb {
...
export function closeFtsDb(fts: FtsDb): void {
  fts.db.close();
}
